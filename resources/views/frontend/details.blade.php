                    ...
                    <div class="prd_details pl-3">
                        ...
                        <div class="prt_02 mb-3">
                            ...
                            <div class="text-left">
                                ...
                                <div class="elis_rty"><span class="ft-medium text-muted line-through fs-md mr-2">BDT
                                        {{ $product_info->first()->price }}</span><span
                                        class="ft-bold theme-cl fs-lg mr-2">BDT
                                    ...
                                    @foreach ($available_colors as $color)
                                        ...
                                        @else
                                            <div class="form-check form-option form-check-inline mb-1">
                                                <input class="form-check-input color_id" type="radio" name="color_id"
                                                    id="white{{ $color->rel_to_color->id }}"
                                                    value="{{ $color->color_id }}">
                                                <label class="form-option-label rounded-circle"
                                                    for="white{{ $color->rel_to_color->id }}"><span
                                ...
                                @if (App\Models\OrderProduct::where('customer_id', Auth::guard('customerlogin')->id())->where('product_id', $product_info->first()->id)->exists())
                                    @if (App\Models\OrderProduct::where('customer_id', Auth::guard('customerlogin')->id())->where('product_id', $product_info->first()->id)->where('review', '!=', null)->first() == false)
                                        <div class="reviews_rate">
                                            <form class="row" method="POST" action="{{ route('review.store') }}">
                                                @csrf
                                    ...
                                    @else
                                        <div class="alert alert-info">You already reviewed</div>
                                    @endif
                                @else
                                    <div class="alert alert-info">You did not purchase this product yet</div>
                                @endif
                            @else
                                <div class="alert alert-info">Please Login to leave a review <a href=""
                                        class="bg-primary px-4 py-2 text-white float-right">Login here</a></div>
                            @endauth
                        </div>
                    </div>
                </div>
                                    ...
                                    <div class="card-body p-0">
                                        <div class="shop_thumb position-relative">
                                            <a class="card-img-top d-block overflow-hidden"
                                                href="{{ route('details', $related->slug) }}"><img class="card-img-top"
                                        ...
                                        <div class="text-left">
                                            <div class="text-center">
                                                <h5 class="fw-bolder fs-md mb-0 lh-1 mb-1"><a
                                                        href="shop-single-v1.html">{{ $related->product_name }}</a></h5>
